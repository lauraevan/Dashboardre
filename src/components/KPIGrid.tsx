    ...
    <div style={{ display: 'flex', alignItems: 'stretch', marginBottom: 20 }}>
      {items.map((item, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'stretch' }}>
          {i > 0 && (
            <div style={{ width: 1, background: 'var(--border)', margin: '2px 34px' }} />
          )}
          <div>
            <div style={{
              fontSize: 10, fontWeight: 500, letterSpacing: '0.13em',
              color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 4, whiteSpace: 'nowrap',
            }}>{item.label}</div>
            <div style={{
              fontSize: item.size, fontWeight: 400, color: 'var(--text)',
              letterSpacing: '-0.02em', lineHeight: 1.05, fontVariantNumeric: 'tabular-nums',
            }}>{item.value}</div>
          </div>
        </div>
      ))}
